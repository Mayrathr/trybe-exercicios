...
// 1 - Imprimir todos os lanches

function imprimirLanches() {
  for( let index = 0; index < menu.sandwiches.length; index += 1) {
    const item = menu.sandwiches[index];
    console.log(`Item: ${item.name} - R$${item.price}`);
  }
  for( let index = 0; index < menu.sideDishes.length; index += 1) {
    const item = menu.sideDishes[index];
    console.log(`Item: ${item.name} - R$${item.price}`);
  }
  for( let index = 0; index < menu.desserts.length; index += 1) {
    const item = menu.desserts[index];
    console.log(`Item: ${item.name} - R$${item.price}`);
  }
  for( let index = 0; index < menu.drinks.length; index += 1) {
    const item = menu.drinks[index];
    console.log(`Item: ${item.name} - R$${item.price}`);
  }
};
imprimirLanches();

// 2 - Pesquisa por nome do lanche e retorna o objeto com todas as informações 

function busca (array, elemento){
  return array.includes(elemento);
}

console.log(busca('Big Mac'));

// 3 - Pesquisa por valor - Lanches que dá pra comprar com o valor passado

// console.log(carteira(100));

// 4- Busca lanches por ingredientes - Retorna um array com todos os lanches que tenham esse ingrediente incluído
